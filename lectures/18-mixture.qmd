...
* Since we have a statistical model, we can choose the number of components/clusters $K$ and the covariance constraint using the Bayesian Information Criterion (BIC) $$\text{BIC} = 2\log L(x \mid \theta) - m\log n \,,$$ where
...
*   In GMMs, we're essentially 
    ...
    * guessing how much do we think each Gaussian component generates each observation
...
colnames(nba_player_probs) <- str_c("cluster", 1:4)
