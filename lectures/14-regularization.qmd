...
get_test_pred <- function(x) {
  ...
  test_x <- as.matrix(select(test_data, -lpsa, -test_fold))           # get test and training matrices
  train_x <- as.matrix(select(train_data, -lpsa, -test_fold))
  
  lm_fit <- lm(lpsa ~ ., data = select(train_data, -test_fold))       # fit models to training data
